/**
 * Player class for FutureskillsArtifact game
 */

class Player {
    constructor(maze, cellSize) {
        // Starting position - find an empty cell in the maze
    ...
    update() {
        ...
        if (this.debug.enabled) {
            const hasMoved = this.x !== this.debug.lastPosition.x || this.y !== this.debug.lastPosition.y;
        ...
        if (this.vx !== 0) {
            ...
            if (nextCellX !== currentCellX) {
                ...
                if (this.debug.enabled) {
                    this.debug.collisionChecks.push({
                        x: this.vx > 0 ? (currentCellX + 1) * this.cellSize : currentCellX * this.cellSize,
                        y: this.y,
                        direction,
                        collision: hasWall
                    });
            ...
            if (canMove) {
                ...
                if (afterMoveX !== currentCellX) {
                    ...
                    if (this.maze.isWall(currentCellX * this.cellSize, this.y, direction)) {
                        this.x = currentCellX * this.cellSize + (direction === 1 ? 
                            this.cellSize - this.width / 3 : this.width / 3);
                    }
                }
            }
        }
        ...
        if (this.vy !== 0) {
            ...
            if (nextCellY !== currentCellY) {
                ...
                if (this.debug.enabled) {
                    this.debug.collisionChecks.push({
                        x: this.x,
                        y: this.vy > 0 ? (currentCellY + 1) * this.cellSize : currentCellY * this.cellSize,
                        direction,
                        collision: hasWall
                    });
                }
            ...
            if (canMove) {
                ...
                if (afterMoveY !== currentCellY) {
                    ...
                    if (this.maze.isWall(this.x, currentCellY * this.cellSize, direction)) {
                        this.y = currentCellY * this.cellSize + (direction === 2 ? 
                            this.cellSize - this.height / 3 : this.height / 3);
                    }
                }
            }
        }
    ...
    render(ctx) {
        ...
        if (!this.isDead) {
            ...
            if (this.effects.damaged.active) {
                ...
                ctx.beginPath();
                ctx.moveTo(this.x - pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight + pulseSize * 0.33);
                ctx.lineTo(this.x + pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight);
                ctx.lineTo(this.x + pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight + pulseSize * 0.66);
                ctx.lineTo(this.x - pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight + pulseSize);
                ctx.closePath();
                ctx.fill();
                ...
                ctx.beginPath();
                ctx.moveTo(this.x - pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight);
                ctx.lineTo(this.x + pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight);
                ctx.lineTo(this.x - pulseSize / 2, this.y - pulseSize / 2 - this.jumpHeight + pulseSize * 0.33);
                ctx.closePath();
                ctx.fill();
            }
    ...
    renderDebug(ctx) {
        ...
        for (const check of this.debug.collisionChecks) {
            ...
            switch (check.direction) {
                ...
                case 1: dx = 10; break;  // right
                case 2: dy = 10; break;  // down
                case 3: dx = -10; break; // left
            }
    ...
    // Add a method to create death effect particles
