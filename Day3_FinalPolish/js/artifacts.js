/**
 * Artifacts class for FutureskillsArtifact game
 */
...
class Artifact {
    ...
    setupArtifactType() {
        switch (this.type) {
            case 'ai':
                ...
                this.description = 'Artificial Intelligence is revolutionizing how we process and analyze data.';
                this.shape = 'cube';
                break;
            case 'cybersecurity':
                this.color = '#ffff00'; // Yellow
                this.name = 'Cybersecurity Artifact';
                this.description = 'Cybersecurity protects systems, networks, and data from digital attacks.';
                this.shape = 'shield';
                break;
            case 'machine-learning':
                this.color = '#00ff00'; // Green
                this.name = 'Machine Learning Artifact';
                this.description = 'Machine Learning enables systems to learn and improve from experience.';
                this.shape = 'sphere';
                break;
            default:
                this.color = '#00ffff'; // Cyan
                this.name = 'Future Skills Artifact';
                this.description = 'This artifact represents an essential future technology skill.';
                this.shape = 'gem';
        }
        
        // Add Rwanda-specific colors as secondary colors
        this.rwandaColors = {
        ...
        // Assign a Rwanda flag color as secondary color
        const colorKeys = Object.keys(this.rwandaColors);
        this.secondaryColor = this.rwandaColors[colorKeys[Math.floor(Math.random() * colorKeys.length)]];
    }
    ...
    getCollisionBox() {
        // Return the artifact's collision box for hit detection
        return {
    ...
    render(ctx) {
        ...
        const gradient = ctx.createRadialGradient(
            this.x, this.y + this.hoverOffset, this.width * 0.25,
            this.x, this.y + this.hoverOffset, this.width + glowSize
        );
        ...
        ctx.globalAlpha = 0.7;
        ctx.beginPath();
        ctx.arc(this.x, this.y + this.hoverOffset, this.width + glowSize, 0, Math.PI * 2);
        ctx.fill();
        
        // Draw artifact based on shape type
        ctx.globalAlpha = 1;
