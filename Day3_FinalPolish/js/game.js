...
class Game {
    constructor() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'F4' && this.debugMode) {
                ...
                console.log(`Show all artifacts: ${this.showAllArtifacts ? 'ON' : 'OFF'}`);
                e.preventDefault();
            }
    ...
    initStage(isNewGame = false) {
        try {
            ...
            console.log(`Initializing stage ${this.stage} with theme "${this.currentTheme.name}"`);
            ...
            this.maze = new Maze(this.width, this.height, this.cellSize, stageDifficulty);
            ...
            this.obstacleManager.generateObstacles(this.maze, this.cellSize, obstacleCount, stageDifficulty);
    ...
    generateBackgroundParticles() {
        ...
        const colors = this.currentTheme ? this.currentTheme.particleColors : ['#00ffff', '#0099ff', '#66ffff'];
    ...
    render() {
        ...
        this.ctx.fillStyle = this.currentTheme ? this.currentTheme.backgroundColor : '#000022';
        ...
        // Display stage theme name
        if (this.currentTheme) {
            this.ctx.fillStyle = 'rgba(255, 255, 255, 0.5)';
            this.ctx.font = '14px "Courier New", monospace';
            this.ctx.textAlign = 'right';
            this.ctx.fillText(`Theme: ${this.currentTheme.name}`, this.width - 10, this.height - 10);
        }
    ...
    renderPauseMenu() {
        ...
        this.ctx.fillText('Press M to return to main menu', this.width / 2, this.height / 2 + 30);
        ...
        this.ctx.fillText('ESC: Pause/Resume', this.width / 2, this.height / 2 + 145);
    ...
    renderGameOver() {
        ...
        this.ctx.fillStyle = '#ffffff';
        this.ctx.font = '18px "Courier New", monospace';
        this.ctx.fillText('Stage: ' + this.stage + ' / ' + this.maxStage, this.width / 2, this.height / 2 - 20);
        this.ctx.fillText('Artifacts collected: ' + this.artifactsCollected + ' / ' + this.totalArtifacts, this.width / 2, this.height / 2 + 10);
        
        if (this.player) {
            this.ctx.fillText('Health: ' + this.player.health + '%', this.width / 2, this.height / 2 + 40);
        }
        ...
        this.ctx.fillStyle = '#ffffff';
        this.ctx.font = '16px "Courier New", monospace';
        this.ctx.fillText('Return to Main Menu', this.width / 2, buttonY + buttonHeight / 2 + 5);
    ...
    completeStage() {
        if (this.stage < this.maxStage) {
            ...
            this.renderStageComplete(() => {
                ...
                console.log(`Transitioned to stage ${this.stage} with theme "${this.currentTheme.name}"`);
            });
        } else {
    ...
    renderStageComplete(callback) {
        ...
        this.ctx.font = 'bold 36px "Courier New", monospace';
        this.ctx.textAlign = 'center';
        this.ctx.fillText('STAGE ' + this.stage + ' COMPLETE!', this.width / 2, this.height / 2 - 80);
        
        // Display theme name and next theme
        const nextTheme = this.themes[this.stage + 1];
        this.ctx.fillStyle = '#ffffff';
        this.ctx.font = '20px "Courier New", monospace';
        this.ctx.fillText(`Theme: ${this.currentTheme.name}`, this.width / 2, this.height / 2 - 120);
        
        if (nextTheme) {
            this.ctx.fillText(`Next: ${nextTheme.name}`, this.width / 2, this.height / 2 - 40);
        }
        
        // Display stage stats
        this.ctx.font = '16px "Courier New", monospace';
        this.ctx.fillText('Time remaining: ' + this.timer + ' seconds', this.width / 2, this.height / 2);
        this.ctx.fillText('Health: ' + this.player.health + '%', this.width / 2, this.height / 2 + 25);
    ...
    renderVictory() {
        ...
        this.ctx.font = 'bold 36px "Courier New", monospace';
        this.ctx.textAlign = 'center';
        this.ctx.fillText('FINAL CERTIFICATION', this.width / 2, this.height / 2 - 120);
        ...
        this.ctx.font = '18px "Courier New", monospace';
        this.ctx.fillText('You have collected all Future Skills Artifacts', this.width / 2, this.height / 2 - 40);
        this.ctx.fillText('and completed your training.', this.width / 2, this.height / 2 - 10);
        ...
        const gradient = this.ctx.createLinearGradient(certX, certY, certX + certWidth, certY + certHeight);
        ...
        this.ctx.fillText('Certificate of Completion', this.width / 2, this.height / 2 + 30);
        this.ctx.fillText('Future Skills Mastery Program', this.width / 2, this.height / 2 + 55);
        
        // Add Rwanda-specific text
        this.ctx.fillStyle = '#E5BE01'; // Rwanda yellow
        this.ctx.font = 'italic 16px "Courier New", monospace';
        this.ctx.fillText('Rwanda Digital Transformation Initiative', this.width / 2, this.height / 2 + 80);
        
        // Display completion stats
        this.ctx.fillStyle = '#ffffff';
        this.ctx.font = '14px "Courier New", monospace';
        this.ctx.fillText('Time remaining: ' + this.timer + ' seconds', this.width / 2, this.height / 2 + 110);
        this.ctx.fillText('Final health: ' + this.player.health + '%', this.width / 2, this.height / 2 + 130);
        this.ctx.fillText(new Date().toLocaleDateString(), this.width / 2, this.height / 2 + 150);
    ...
    cycleTargetArtifact() {
        if (!this.debugMode || !this.showShortestPath || !this.artifacts || this.artifacts.length === 0) return;
        
        this.targetArtifactIndex = (this.targetArtifactIndex + 1) % this.artifacts.length;
        this.updateShortestPath();
        
        console.log(`Target artifact: ${this.targetArtifactIndex + 1}/${this.artifacts.length}`);
    }
    
    // Update the shortest path to the current target artifact
    updateShortestPath() {
        if (!this.debugMode || !this.showShortestPath || !this.player || !this.maze || !this.artifacts) return;
        
        if (this.targetArtifactIndex >= 0 && this.targetArtifactIndex < this.artifacts.length) {
    ...
    renderDebugInfo() {
        ...
        y += 20;
        this.ctx.fillText(`FPS: ${Math.round(1000 / (performance.now() - this.lastTime))}`, 20, y);
        y += 20;
        this.ctx.fillText(`Player: (${Math.round(this.player.x)}, ${Math.round(this.player.y)})`, 20, y);
        y += 20;
        this.ctx.fillText(`Grid: (${Math.floor(this.player.x / this.cellSize)}, ${Math.floor(this.player.y / this.cellSize)})`, 20, y);
        y += 20;
        this.ctx.fillText(`Shortest Path: ${this.showShortestPath ? 'ON (F2)' : 'OFF (F2)'}`, 20, y);
        
        if (this.showShortestPath) {
            y += 20;
            this.ctx.fillText(`Target Artifact: ${this.targetArtifactIndex + 1}/${this.artifacts.length} (F3)`, 20, y);
        }
        
        // Show player debug status
        y += 20;
        this.ctx.fillText(`Player Debug: ${this.player.debug.enabled ? 'ON (B)' : 'OFF (B)'}`, 20, y);
        
        // Show artifact visibility status
        y += 20;
        this.ctx.fillText(`Show Artifacts: ${this.showAllArtifacts ? 'ON (F4)' : 'OFF (F4)'}`, 20, y);
        
        // Show obstacle visibility status
        y += 20;
        this.ctx.fillText(`Show Obstacles: ${this.showObstacles ? 'ON (F5)' : 'OFF (F5)'}`, 20, y);
