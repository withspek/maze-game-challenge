/**
 * Maze generator for FutureskillsArtifact game
 */
...
class Maze {
    ...
    generateMaze() {
        ...
        const startX = getRandomInt(0, this.cols - 1);
        const startY = getRandomInt(0, this.rows - 1);
        ...
        while (stack.length > 0) {
            ...
            if (neighbors.length > 0) {
                ...
                const nextIndex = getRandomInt(0, neighbors.length - 1);
    ...
    addLoops(difficulty) {
        ...
        for (let i = 0; i < numLoops; i++) {
            ...
            if (wallIndex === 0 && y > 0) { // Top
                this.grid[y - 1][x].walls[2] = false;
            } else if (wallIndex === 1 && x < this.cols - 1) { // Right
                this.grid[y][x + 1].walls[3] = false;
            } else if (wallIndex === 2 && y < this.rows - 1) { // Bottom
                this.grid[y + 1][x].walls[0] = false;
            } else if (wallIndex === 3 && x > 0) { // Left
    ...
    isWall(x, y, direction) {
        ...
        try {
            ...
            console.error(`Error checking wall at ${gridX},${gridY}, direction ${direction}:`, error);
    ...
    render(ctx, theme) {
        ...
        for (let y = 0; y < this.rows; y++) {
            for (let x = 0; x < this.cols; x++) {
                ...
                if ((x + y) % 2 === 0) {
                    ctx.beginPath();
                    ctx.moveTo(cellX + this.cellSize * 0.25, cellY + this.cellSize * 0.25);
                    ctx.lineTo(cellX + this.cellSize * 0.75, cellY + this.cellSize * 0.75);
                    ctx.stroke();
                    
                    ctx.beginPath();
                    ctx.moveTo(cellX + this.cellSize * 0.75, cellY + this.cellSize * 0.25);
                    ctx.lineTo(cellX + this.cellSize * 0.25, cellY + this.cellSize * 0.75);
                    ctx.stroke();
                }
    ...
    findShortestPath(startX, startY, endX, endY) {
        ...
        if (startX < 0 || startX >= this.cols || startY < 0 || startY >= this.rows ||
            endX < 0 || endX >= this.cols || endY < 0 || endY >= this.rows) {
            return [];
        }
