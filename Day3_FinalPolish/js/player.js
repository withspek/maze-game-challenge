...
class Player {
    ...
    handleKeyDown(e) {
        switch (e.key) {
            ...
            case ' ': // Spacebar
                ...
                e.preventDefault();
                break;
            case 'd': // Toggle debug mode (use 'b' to avoid conflict with direction)
            case 'b':
    ...
    renderDebug(ctx) {
        ...
        let textY = box.y - 55;
        ctx.fillText(`Pos: (${Math.round(this.x)}, ${Math.round(this.y)})`, box.x, textY);
        textY += 15;
        ctx.fillText(`Cell: (${cellX}, ${cellY})`, box.x, textY);
        textY += 15;
        ctx.fillText(`Vel: (${this.vx.toFixed(1)}, ${this.vy.toFixed(1)})`, box.x, textY);
        textY += 15;
        ...
        for (const dir of directions) {
            ...
            if (hasWall) {
                ...
                const wallX = cellX * this.cellSize + (dir.dx === 1 ? this.cellSize : 0);
                const wallY = cellY * this.cellSize + (dir.dy === 1 ? this.cellSize : 0);
